...
<table id="tabla-1" class="table table-bordered" cellspacing="0" width="100%">
 ...
 if(isset($_SESSION['filtro'])){
      ...
      $sql="SELECT * FROM mantenimientos WHERE tipo_vehiculo=2 ".$fvehiculo." ".$ftipo." ".$ffecha." ".$festatus." ORDER BY id_mantenimiento DESC LIMIT 300";
      $sq= $db->query($sql);
      $rows=$sq->fetchAll();
      foreach ($rows as $row) {
      ...
      $sql="SELECT * FROM motos WHERE id_moto='$id_unidad'";
      $sq= $db->query($sql);
      $rows=$sq->fetchAll();
      foreach ($rows as $row) {
       $nombre=$row["nombre"];
