...
<table id="tabla-1" class="table table-bordered" cellspacing="0" width="100%">
  ...
  <tbody>
    <? 
      ...
      $sql="SELECT * FROM mantenimientos WHERE id_vehiculo='$id_unidad' and tipo_vehiculo=1 ORDER BY id_mantenimiento DESC LIMIT 1";
      $sq= $db->query($sql);
      $rows=$sq->fetchAll();
      foreach ($rows as $row) {
     ...
     if($estatus==0){
      ...
      $pdf='<form target="_blank" action="pdf/mantenimientoE.php" method="POST"><input name="id" value="'.$id_mantenimiento.'" style="display:none;"><button class="btn btn-danger btn-sm btn-rounded" type="submit">
       ...
       $editar='<button class="btn btn-primary btn-sm  btn-rounded" data-toggle="modal" data-target="#mod_vehiculo" data-whatever="@mdo" id="modificar_veh"
       id_unidad="'.$id_unidad.'"
       nombre="'.$nombre.'"
       placas="'.$placas.'"
       marca="'.$marca.'"
