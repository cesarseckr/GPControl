...
function add_cat_servicios(){
  var nombre =$("#add_cat_nombre").val();
  var categoria= $("#add_cat_categoria").val();
  var tipo=$("#add_cat_tipo").val();

  alerta_cargando("Cargando","Por favor espere a que se ejecuten los registros");
  $.ajax({
    url:"php/add_cat_serviciosE.php",
    method:"POST",
    data:{nombre:nombre, 
      categoria: categoria,
      tipo: tipo},
...
function mod_cat_servicios(){
  var id_catalogo =$("#mod_id_catalogo").val();
  var nombre =$("#mod_cat_nombre").val();
  var categoria= $("#mod_cat_categoria").val();
  var tipo=$("#mod_cat_tipo").val();

  alerta_cargando("Cargando","Por favor espere a que se ejecuten los registros");
  $.ajax({
    url:"php/mod_cat_serviciosE.php",
    method:"POST",
    ...
    success: function(data){
      if(data==1){
        ...
        $('#mod_cat_servicios').modal('toggle');  
      }else{
        setTimeout('alerta_error("Error","A ocurrido un error al tratar de registrar");', timer);  
      }
    } 
...
$(document).on('click', '#modificar_cat_servicios',function(){
  $("#mod_id_catalogo").val($(this).attr('id_catalogo'));
  $("#mod_cat_nombre").val($(this).attr('nombre'));
  $("#mod_cat_categoria").val($(this).attr('categoria'));
  $("#mod_cat_tipo").val($(this).attr('tipo'));
});

$(document).on('click', '#del_cat_servicios',function(){
  var titulo="Eliminar servicio del catálogo.";
  var mensaje="¿Seguro que desea eliminar este servicio del catálogo?";
  var mensaje_ok="Servicio eliminado del catálogo correctamente";
  var mensaje_no="No se puede eliminar este servicio";
  var listado="Catálogo de servicios";
  var id=$(this).attr('id_catalogo');
  var ruta="php/del_cat_serviciosE.php";
  alerta_confirmacion(titulo,mensaje,mensaje_ok,mensaje_no,listado,id,ruta);
});
