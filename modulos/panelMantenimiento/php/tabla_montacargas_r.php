...
<table id="tabla-1" class="table table-bordered" cellspacing="0" width="100%">
  ...
  <tbody>
    <? 
      ...
      $sql="SELECT * FROM mantenimientos WHERE tipo_vehiculo=3 ".$fvehiculo." ".$ftipo." ".$ffecha." ".$festatus." ORDER BY id_mantenimiento DESC LIMIT 300";
      $sq= $db->query($sql);
      $rows=$sq->fetchAll();
      foreach ($rows as $row) {
      ...
      $sql="SELECT * FROM montacargas WHERE id_montacargas='$id_unidad'";
      $sq= $db->query($sql);
      $rows=$sq->fetchAll();
      foreach ($rows as $row) {
       $nombre=$row["nombre"];
...
if($estatus==1){
      ...
      $pdf='<form target="_blank" action="pdf/mantenimientoE.php" method="POST"><input name="id" value="'.$id_mantenimiento.'" style="display:none;"><button class="btn btn-danger btn-sm btn-rounded" type="submit">
       ...
       $foto='<button class="btn btn-primary btn-sm  btn-rounded" data-toggle="modal" data-target="#add_imagen_r" data-whatever="@mdo" id="add_imagen"
