...
<table id="tabla-1" class="table table-bordered" cellspacing="0" width="100%">
  ...
  <tbody>
    <?
      ...
      $sql="SELECT * FROM mantenimientos WHERE tipo_vehiculo=1 ".$fvehiculo." ".$ftipo." ".$ffecha." ".$festatus." ORDER BY id_mantenimiento DESC LIMIT 300";
      $sq= $db->query($sql);
      $rows=$sq->fetchAll();
      foreach ($rows as $row) {
      ...
      $sql="SELECT * FROM dolly WHERE id_dolly='$id_unidad'";
      $sq= $db->query($sql);
      $rows=$sq->fetchAll();
      foreach ($rows as $row) {
       $marca=$row["marca"];
